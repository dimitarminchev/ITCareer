...
FROM `soft_uni`.`employees` AS e 
WHERE e.`employee_id` IN (SELECT DISTINCT manager_id FROM `soft_uni`.`employees`)
  AND (SELECT `manager_id` FROM `soft_uni`.`employees` WHERE `manager_id` = e.`employee_id` LIMIT 4,1) IS NOT NULL
  AND (SELECT `manager_id` FROM `soft_uni`.`employees` WHERE `manager_id` = e.`employee_id` LIMIT 5,1) IS NULL
